...
# 🔑 مفتاح التشفير (غيره قبل التوزيع التجاري)
...
def create_license_files(device_id: str, activated: bool = False):
    license_data = {"device_id": device_id, "activated": activated, "created_at": platform.node(), "timestamp": str(int(time.time()))}
    _safe_write_file(LICENSE_FILE, _encrypt_license_data(license_data))
    
    sfo_data = {"format": "SFO_BACKUP", "version": 1, "encrypted_device_id": _encrypt_id_for_sfo(device_id), "checksum": hashlib.sha256(device_id.encode()).hexdigest()}
...
def read_license() -> Optional[dict]:
    ...
    try:
        with open(LICENSE_FILE, 'rb') as f: return _decrypt_license_data(f.read())
    except: return None
...
def update_license_status(activated: bool):
    data = read_license()
    if data is None:
        create_license_files(get_device_id(), activated=activated)
        return
...
def check_license() -> Tuple[bool, str, Optional[str]]:
    ...
    if not LICENSE_FILE.exists():
        if not INIT_MARKER.exists():
            ...
            INIT_MARKER.touch()
            if os.name == 'nt': os.system(f'attrib +h "{INIT_MARKER}" >nul 2>&1')
            return True, current_id, None
        return False, current_id, "License files have been removed or corrupted."

    data = read_license()
    if data is None: return False, current_id, "License file is corrupted or unreadable."
    if data.get("device_id") != current_id:
        data["device_id"] = current_id
        update_license_status(True)
        return True, current_id, None
    return True, current_id, None
...
def import_sfo_file(file_path: str) -> Tuple[bool, str]:
    current_id = get_device_id()
    try:
        with open(file_path, 'r', encoding='utf-8') as f: sfo = json.load(f)
        if sfo.get("format") != "SFO_BACKUP": return False, "Invalid SFO file format."
        decrypted_id = _decrypt_id_from_sfo(sfo.get("encrypted_device_id", ""))
        if not decrypted_id: return False, "Failed to decrypt device ID."
        if hashlib.sha256(decrypted_id.encode()).hexdigest() != sfo.get("checksum"): return False, "SFO integrity check failed."
        if decrypted_id != current_id: return False, "SFO file belongs to a different device."
        update_license_status(True)
        return True, ""
    except Exception as e: return False, f"Error reading SFO file: {str(e)}"
...
class ActivationDialog(QDialog):
    def __init__(self, device_id: str, reason: str, expected_path: str):
        ...
        lay.addWidget(QLabel(f"Expected directory: <code>{expected_path}</code>"))
        ...
        self.code_in.setPlaceholderText("e.g., A1B2C3D4E5F6G7H8")
    ...
    def _handle_sfo(self):
        if not self.selected_sfo: return
